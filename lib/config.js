 ...
 *  time (UTC+8): 09:00–12:00 and 14:00–18:00; off-peak prices are half. */
...
export const PrefsSchema = z.object({
  ...
  prices: z.object({
    /** Peak windows as ["HH:MM", "HH:MM"] pairs, Beijing time (UTC+8). */
    peakWindows: z.array(z.array(z.string())).default([
      ["09:00", "12:00"],
      ["14:00", "18:00"],
    ]),
    /** Multiplier applied outside peak windows (DeepSeek: 0.5). */
    offPeakFactor: z.number().min(0).max(1).default(0.5),
    /** Per-model CNY-per-1M-token prices at PEAK rates. */
    models: z.dict(z.object({
      input: z.number().min(0).default(3.0),
      cacheHit: z.number().min(0).default(0.1),
      output: z.number().min(0).default(9.0),
 ...
 * midnight.
 */
export function isPeak(timeMs, peakWindows) {
...
export function peakStatus(timeMs, peakWindows) {
  return isPeak(timeMs, peakWindows) ? "peak" : "off-peak";
}

/**
 * Cost (CNY) of one provider usage record.
 * @param usage - provider usage: inputTokens (total input incl. cache reads),
...
export function requestCost(usage, model, timeMs, prices) {
  ...
  const factor = isPeak(timeMs, prices?.peakWindows) ? 1 : (prices?.offPeakFactor ?? 0.5);
  const uncached = Math.max(0, input - cacheRead);
  const cost = ((uncached * entry.input + cacheRead * entry.cacheHit + output * entry.output) / 1_000_000) * factor;
  return {
    cost,
    peak: factor === 1,
    tokens: { uncached, cacheRead, output },
  };
}
