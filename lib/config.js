 ...
 *
 * The DeepSeek platform's per-key usage dashboard ("消耗" figures) matches
 * the same formula with the cache-hit rate effectively zero: the daily
 * per-key amount reported there equals uncached input + output only. This
 * plugin's figures are calibrated to the platform dashboard (the reference
 * the user checks), so `CACHE_HIT_CHARGED` stays false; flip it to true to
 * charge the published hit rate if the dashboard starts including it.
 */
const CACHE_HIT_CHARGED = false;
