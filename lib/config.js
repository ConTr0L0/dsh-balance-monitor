...
import { isCnHoliday } from "./holidays.js";
 ...
 *
 * Official 2026-09-10 price list: deepseek-flash (DeepSeek-V4.1-Flash) replaced
 * deepseek-v4-flash at lower prices. Per the official footnote, the legacy ids
 * deepseek-v4-flash / deepseek-v4-flash-vision-exp are still callable but are
 * served — and billed — by V4.1-Flash, and temporary ids such as
 * deepseek-v4.1-flash-expires-on-0910 are Flash variants too: they all fall
 * through to DEFAULT_MODEL_PRICE, which is deliberately the Flash price. */
export const DEFAULT_MODELS = {
  "deepseek-flash": { input: 2.0, cacheHit: 0.04, output: 8.0 },
  "deepseek-v4-pro": { input: 9.0, cacheHit: 0.3, output: 27.0 },
};

/** Fallback pricing for any model not present in the table (Flash peak price). */
export const DEFAULT_MODEL_PRICE = { input: 2.0, cacheHit: 0.04, output: 8.0 };

/**
 * Officially announced model reroutes: from `since` (epoch ms) on, requests
 * named `model` are served by `to` and billed at its price. Source: pricing
 * page footnote (2) captured 2026-09-10 — after Beijing 2026-09-14 12:00 and
 * until a future V4.1 Pro launch, deepseek-v4-pro requests route to
 * V4.1-Flash at Flash prices.
 */
const MODEL_REROUTES = [
  { model: "deepseek-v4-pro", since: Date.parse("2026-09-14T12:00:00+08:00"), to: "deepseek-flash" },
];

/** Resolve officially announced reroutes for one request timestamp. */
function rerouteModel(model, timeMs) {
  if (model) {
    for (const rule of MODEL_REROUTES) {
      if (rule.model === model && timeMs >= rule.since) return rule.to;
    }
  }
  return model;
}
...
/**
 * Peak-window check (Beijing time). Official rule (pricing-page footnote,
 * re-verified live 2026-09-20): the peak windows apply Monday-Friday EXCLUDING
 * Chinese statutory holidays; weekends and statutory holidays are all-day
 * off-peak. weekendOffPeak / holidayOffPeak gate the two exemptions (when a page
 * stops stating one, its flag is false and only the remaining rule applies).
 *
 * A 调休上班 weekend stays off-peak: the rule keys on the calendar weekend,
 * never on the Chinese work roster.
 */
export function isPeak(timeMs, peakWindows, weekendOffPeak, holidayOffPeak) {
  if (weekendOffPeak && isWeekend(timeMs)) return false;
  if (holidayOffPeak && isCnHoliday(timeMs)) return false;
...
export function peakStatus(timeMs, peakWindows, weekendOffPeak, holidayOffPeak) {
  return isPeak(timeMs, peakWindows, weekendOffPeak, holidayOffPeak) ? "peak" : "off-peak";
}
...
export function requestCost(usage, model, timeMs, prices) {
  ...
  const routed = rerouteModel(model, timeMs);
  const entry = (routed && table[routed]) || (model && table[model]) || DEFAULT_MODEL_PRICE;
  const factor = isPeak(timeMs, prices?.peakWindows, prices?.weekendOffPeak, prices?.holidayOffPeak)
    ? 1
    : (prices?.offPeakFactor ?? 0.5);
