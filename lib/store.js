...
/** v8: official 2026-09-10 price list — Flash repriced (3/0.1/9 → 2/0.04/8),
 * legacy/temp Flash ids fall to the Flash price, and the announced
 * deepseek-v4-pro → Flash reroute (Beijing 2026-09-14 12:00) encoded. Full
 * recompute required; also adapts to 0.1.5's session.v3.jsonl.zstd logs. */
...
export function emptyState() {
  return {
    ...
    savedAt: 0,
    pricing: null,
    // Pending pricing-change popup: { fetchedAt, old, new } or null. Set by the
    // official-pricing sync when the published table changed; cleared by the
    // client's pricing-notice/ack after the user has seen the diff once.
    pricingNotice: null,
    // message.id → 1 for every already-counted completion. DSH re-writes the
    // same session content into several session files (child/surface copies),
    // so completions must be deduplicated by their stable message id.
    seenIds: {},
  };
}
...
export function loadState(home) {
  ...
  try {
    ...
    return {
      ...
      pricingNotice: raw.pricingNotice ?? null,
      seenIds: raw.seenIds ?? {},
    };
  } catch {
    return emptyState();
