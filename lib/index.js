...
import { fetchProviderBalanceWithRetry } from "./balance.js";
import { emptyState, loadState, saveState } from "./store.js";
import { defaultPricing, fetchOfficialPricing, sanitizePricing, diffPricing } from "./pricing.js";
...
/** Re-arm a failed balance poll this soon (capped by refreshInterval). */
const BALANCE_RETRY_DELAY_MS = 10_000;
/** Same, while nothing has been shown yet — the empty plate is worth a retry sooner. */
const BALANCE_COLD_RETRY_DELAY_MS = 3000;
/** Skip a provider whose last attempt is younger than this. */
const BALANCE_MIN_INTERVAL_MS = 4000;
...
export function apply(ctx, _config = {}) {
  ...
  const monitor = {
    ...
    /** In-flight balance run shared by the poll, the manual refresh and retries. */
    balanceInflight: null,
    /** One-shot timer re-arming a failed balance poll. */
    balanceRetryTimer: null,
    syncing: false,
    saveTimer: null,
    /** unrecognized session log names already warned about (dedupe key: "id::name") */
    unknownLogNames: new Set(),
  ...
  async function syncStats() {
    ...
    try {
      const files = listSessionFiles(join(monitor.home, "sessions"), (sessionId, fileName) => {
        const key = `${sessionId}::${fileName}`;
        if (monitor.unknownLogNames.has(key)) return;
        monitor.unknownLogNames.add(key);
        ctx.logger.warn(
          "dsh-balance-monitor: unrecognized session log format %s in session %s — update the plugin or its usage is under-counted",
          fileName,
          sessionId,
        );
      });
      ...
      for (const entry of files) {
        ...
        if (syncSessionFile(session, monitor.state, currentPrices(), entry.file, entry.fileName)) changed = true;
      }
  ...
  async function refreshOfficialPricing() {
    ...
    if (result.ok) {
      const previous = monitor.pricing;
      monitor.pricing = result.pricing;
      monitor.state.pricing = result.pricing;
      // When the published rules changed against the last officially synced
      // table, queue a one-shot popup with the before/after diff. The client
      // shows it on its next overview fetch and acks it once seen. Only a
      // genuine deepseek-docs snapshot counts as "previous" — the built-in
      // defaults are fallback values, not a rule change.
      if (previous?.source === "deepseek-docs" && diffPricing(previous, result.pricing).any) {
        monitor.state.pricingNotice = {
          fetchedAt: Date.now(),
          old: sanitizePricing(previous),
          new: sanitizePricing(result.pricing),
        };
        ctx.logger.info("dsh-balance-monitor: official pricing changed — notice queued for display");
      }
      persistSoon();
      ctx.logger.info("dsh-balance-monitor: official pricing synced (%d models)", Object.keys(result.pricing.models).length);
      // ponytail: once per daily sync - a real reminder to extend the table.
      const bjYear = new Date(Date.now() + 8 * 3600_000).getUTCFullYear();
      if (!holidayYears().includes(bjYear)) {
        ctx.logger.warn(
          "dsh-balance-monitor: no Chinese holiday table for %d - statutory holidays bill at peak prices; add it to lib/holidays.js",
          bjYear,
        );
      }
    } else {
  ...
  /**
   * Publish a failed poll without discarding the last known balance.
   *
   * A momentary provider outage (or a local TLS scanner breaking the
   * connection) must not blank the sidebar: the last successful snapshot is
   * kept, flagged `stale`, and re-published until a poll succeeds again.
   */
  function failedBalanceEntry(previous, result) {
    const lastGood = previous !== undefined && (previous.ok === true || typeof previous.total === "number") ? previous : null;
    return {
      ...(lastGood ?? {}),
      ...result,
      ok: false,
  ...
  /**
   * Re-arm a failed poll well before the next scheduled tick. Only transport
   * failures qualify: a rejected credential or request (401/403) must not
   * hammer the provider every few seconds.
   */
  function scheduleBalanceRetry(cold = false) {
    if (monitor.balanceRetryTimer !== null) return;
    const ceiling = Math.min(BALANCE_RETRY_DELAY_MS, (monitor.prefs.refreshInterval ?? 60) * 1000);
    const delay = cold ? Math.min(BALANCE_COLD_RETRY_DELAY_MS, ceiling) : ceiling;
    monitor.balanceRetryTimer = setTimeout(() => {
      monitor.balanceRetryTimer = null;
      void refreshBalance(true);
    }, delay);
    if (monitor.balanceRetryTimer.unref) monitor.balanceRetryTimer.unref();
  }

  /**
   * Refresh every configured provider's balance. Concurrent callers — the poll
   * tick, the manual refresh button, a retry — share one in-flight run, so a
   * burst of clicks cannot stack provider requests.
   */
  async function refreshBalance(force = false) {
    if (monitor.balanceInflight !== null) return monitor.balanceInflight;
    monitor.balanceInflight = refreshBalanceOnce(force).finally(() => {
      monitor.balanceInflight = null;
    });
    return monitor.balanceInflight;
  }

  async function refreshBalanceOnce(force) {
    for (const provider of Object.keys(monitor.prefs.providers ?? {})) {
      const cached = monitor.balanceCache.get(provider);
      if (!force && cached?.attemptedAt && Date.now() - cached.attemptedAt < BALANCE_MIN_INTERVAL_MS) continue;
      ...
      const config = monitor.prefs.providers[provider] ?? {};
      try {
        const result = await fetchProviderBalanceWithRetry(provider, config.baseURL, key);
        if (result.ok) {
          if (cached?.ok === false) ctx.logger.info("dsh-balance-monitor: %s balance restored", provider);
          monitor.balanceCache.set(provider, {
            ...result,
            fetchedAt: Date.now(),
            attemptedAt: Date.now(),
          ...
          if (cached?.ok !== false) {
            ctx.logger.warn("dsh-balance-monitor: %s balance fetch failed: %s", provider, result.error);
          }
          const entry = failedBalanceEntry(cached, result);
          monitor.balanceCache.set(provider, entry);
          if (result.retryable !== false) scheduleBalanceRetry(typeof entry.total !== "number");
        }
      } catch (error) {
        const message = error instanceof Error ? error.message : String(error);
        if (cached?.ok !== false) {
          ctx.logger.warn("dsh-balance-monitor: %s balance fetch failed: %s", provider, message);
        }
        const entry = failedBalanceEntry(cached, { ok: false, error: message, retryable: true });
        monitor.balanceCache.set(provider, entry);
        scheduleBalanceRetry(typeof entry.total !== "number");
      }
    }
  }
  ...
  ctx.inject(["settings", "credentials"], (sctx) => {
    ...
    return () => {
      if (timer !== null) clearInterval(timer);
      if (monitor.balanceRetryTimer !== null) clearTimeout(monitor.balanceRetryTimer);
        ...
        switch (endpoint) {
          case "overview": {
            return rpcOk({
              ...
              pricingNotice: monitor.state.pricingNotice ?? null,
          ...
          case "pricing-notice/ack": {
            // The user has seen the before/after popup; clear it for good.
            monitor.state.pricingNotice = null;
            persistSoon();
