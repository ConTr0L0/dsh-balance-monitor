...
- **Popover 详情面板**：点击余额打开。
  ...
  - 全部会话消耗列表（按最近排序）
- **峰谷计价**：内置 DeepSeek 官方规则（北京时间高峰 9:00–12:00、14:00–18:00，谷时半价），价目表预填官方 V4 价格（可编辑、可增删模型），成本按每个请求的实际发生时间与模型精确计算。
...
- 请求发生时间落在高峰窗口内按价目表原价，否则乘以「谷时折扣」（默认 0.5）。
- 未知模型走价格表 `默认条目`（flash 价），可在设置中覆盖。
