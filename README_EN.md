...
- **Peak/off-peak pricing**: DeepSeek rules built in (Beijing-time peak windows 09:00–12:00 & 14:00–18:00, off-peak = half price); **the price table is verified automatically from the official DeepSeek docs page once a day** (billing rules are plugin-managed, never user-entered); costs are computed per request using the exact model and request time. On sync failure a built-in default table is used.
